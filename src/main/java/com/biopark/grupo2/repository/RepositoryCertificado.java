...
public interface RepositoryCertificado extends JpaRepository<Certificado, Long> {
    @Query(value = "SELECT DISTINCT f.base, r.ultimaMod AS data, " +
                   "f.titulo AS form, " +
                   ...
                   "AND (f.id_formulario, r.ultimaMod) " +
                   "IN (SELECT r2.id_formulario, MAX(r2.ultimaMod) " +
    ...
    @Query(value = "SELECT DISTINCT f.base, r.ultimaMod AS data, " +
                   ...
                   "JOIN resposta r ON r.id_formulario = f.id_formulario " +
                   "JOIN usuario u ON r.id_usuario = u.id_usuario " +
                   "WHERE (f.id_formulario, r.ultimaMod) " +
                   "IN (SELECT r2.id_formulario, MAX(r2.ultimaMod) " +
                   "FROM resposta r2 GROUP BY r2.id_formulario) " +
    ...
    @Query(value = "SELECT DISTINCT f.base, r.ultimaMod AS data, " +
                   ...
                   "JOIN resposta r ON r.id_formulario = f.id_formulario " +
                   "JOIN usuario u ON r.id_usuario = u.id_usuario " +
                   "WHERE f.base = :status AND (f.id_formulario, r.ultimaMod) " +
                   "IN (SELECT r2.id_formulario, MAX(r2.ultimaMod) " +
                   "FROM resposta r2 " +
                   "GROUP BY r2.id_formulario)",
                    nativeQuery = true)
